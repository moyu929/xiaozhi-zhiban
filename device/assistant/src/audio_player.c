...
int audio_player_init(audio_player_t *player, int sample_rate, int channels) {
    ...
    if (!player->decode_buf || !player->s32_buf) {
        ...
        if (player->opus_decoder) opus_decoder_destroy((OpusDecoder *)player->opus_decoder);
...
int audio_player_write_opus(audio_player_t *player, const uint8_t *opus_data, size_t opus_len, uint32_t timestamp) {
    ...
    while (offset < samples) {
        ...
        if (chunk > S32_BUF_SAMPLES) chunk = S32_BUF_SAMPLES;
...
void audio_player_reset_decoder(audio_player_t *player) {
    ...
    if (player->opus_decoder) {
        opus_decoder_destroy((OpusDecoder *)player->opus_decoder);
        int error;
        player->opus_decoder = opus_decoder_create(
            player->sample_rate, player->channels, &error);
        if (!player->opus_decoder || error != OPUS_OK) {
            PLOG_E("PLAYER", "解码器重置失败: %d", error);
        }
    }
    ...
    /* 清空时间戳队列 */
    if (player->ts_queue) {
        pthread_mutex_lock(&player->ts_queue->mutex);
        player->ts_queue->head = 0;
        player->ts_queue->count = 0;
        pthread_mutex_unlock(&player->ts_queue->mutex);
    }
...
int audio_player_set_sample_rate(audio_player_t *player, int new_sample_rate) {
    ...
    if (player->sample_rate == new_sample_rate) return 0;
    ...
    /* 创建新采样率的解码器 */
    int error;
    player->opus_decoder = opus_decoder_create(new_sample_rate, player->channels, &error);
    if (!player->opus_decoder || error != OPUS_OK) {
...
int audio_player_set_volume(audio_player_t *player, int volume) {
    if (!player || !player->track_handle) return -1;
    if (volume < 0) volume = 0;
    if (volume > 100) volume = 100;
    int track_vol = volume * 80 / 100;
    return audio_track_set_volume(player->track_handle, track_vol);
}
