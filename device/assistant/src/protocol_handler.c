    ...
    if (binary) {
        ...
        if (len >= sizeof(binary_header_v2_t)) {
            ...
            if ((version == 2 || (version == 0 && type <= 1 && payload_size > 0 && payload_size + sizeof(binary_header_v2_t) == len)) && payload_size > 0 && payload_size < len) {
                ...
                if (payload_size > PROTO_MAX_AUDIO_PAYLOAD) payload_size = PROTO_MAX_AUDIO_PAYLOAD;
                ...
                size_t copy_len = len;
                if (copy_len > PROTO_MAX_AUDIO_PAYLOAD) copy_len = PROTO_MAX_AUDIO_PAYLOAD;
...
int protocol_handler_init(protocol_handler_t *proto, protocol_config_t *config) {
    ...
    PLOG_I("PROTO", "初始化完成 (AEC=%s, 采样率=%d, 帧时长=%d)",
           config->aec_enabled ? "开启" : "关闭", config->sample_rate, config->frame_duration);
...
int protocol_handler_poll(protocol_handler_t *proto, int timeout_ms) {
    if (!proto || !proto->connected) return -1;
...
int protocol_handler_send_audio(protocol_handler_t *proto, const uint8_t *opus_data, size_t opus_len) {
    if (!proto || !opus_data || opus_len == 0) return -1;
    if (!proto->connected || !proto->hello_received) return -1;
