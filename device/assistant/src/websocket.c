...
static int ws_parse_url(websocket_t *ws)
{
    ...
    if (colon && (!slash || colon < slash))
    {
        ...
        if (host_len >= sizeof(ws->host))
            host_len = sizeof(ws->host) - 1;
...
static int ws_send_raw(websocket_t *ws, const uint8_t *data, size_t len)
{
    if (!ws)
        return -1;
    if (!ws->secure && ws->sockfd < 0)
        return -1;

    if (ws->secure)
    {
        /* TLS加密连接发送 */
        if (!tls_transport_is_connected(&ws->tls))
            return -1;
...
static int ws_process_recv_buffer(websocket_t *ws)
{
    ...
    while (consumed < buf_len)
    {
        ...
        if (masked)
        {
            if (buf_len - consumed < header_size + 4)
                break;
        ...
        /* 检查是否已接收完整的帧数据 */
        if (consumed + header_size + payload_len > buf_len)
            break;
...
{
    if (!ws || ws->header_count >= WS_MAX_HEADERS)
        return;
