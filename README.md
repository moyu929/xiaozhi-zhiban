...
<p align="center">
  <strong>智伴教育机器人1X（GS705B）的开源语音助手替代方案 — 开箱即用，直接部署</strong>
</p>
...
小智·智伴是一个为**智伴教育机器人1X**（内部型号 GS705B）开发的开源语音助手替代方案。它替换设备原版的 `sair` 语音助手进程，提供更灵活的云端对接能力和设备管理功能。
...
- **唤醒词不可自定义** — 本项目使用设备原生唤醒词模型，暂不支持更改唤醒词
- **无实时对话模式** — 设备原生 AEC（回声消除）为闭源，无法在自定义程序中调用，因此仅支持 AutoStop 模式（TTS 播放时停止录音，播放结束后恢复）
...
设备原生包含 AEC（回声消除）模型，但该功能封装在闭源引擎（libduilite_fespl.so）内部，无公开 API 可供调用，因此 Realtime（实时对话）模式暂不可用。若未来能逆向还原 AEC 接口，则可支持实时对话。
