...
#define LOAD_SYM(h, name, type) do { \
    mcp->name = (type)dlsym(h, #name); \
    if (!mcp->name) PLOG_W("MCP", "符号未找到: %s", #name); \
} while(0)
...
static int exec_tool(mcp_handler_t *mcp, const char *name, const char *args_json, size_t args_len, char *result, int result_size) {
    ...
    if (strcmp(name, "self.get_device_status") == 0) {
        ...
        if (mcp->sound_get_sys_volume) vol = mcp->sound_get_sys_volume();
        if (mcp->lcd_get_backlight) backlight = mcp->lcd_get_backlight();
        if (mcp->power_get_charge_status) mcp->power_get_charge_status(&charge_st);
        if (mcp->power_get_battery_cap) battery = mcp->power_get_battery_cap();
        ...
        if (lf) { fgets(loadbuf, sizeof(loadbuf), lf); fclose(lf); }
    ...
    if (strcmp(name, "self.audio_speaker.set_volume") == 0) {
        ...
        if (mcp->sound_set_sys_volume) mcp->sound_set_sys_volume(vol * 40 / 100);
    ...
    if (strcmp(name, "self.screen.set_brightness") == 0) {
        ...
        if (mcp->lcd_set_backlight) mcp->lcd_set_backlight(b);
    ...
    if (strcmp(name, "self.reboot") == 0) {
        ...
        PLOG_I("MCP", "收到重启请求");
        if (mcp->sound_tts_play) mcp->sound_tts_play(10);
        system("reboot");
        return 0;
    }
...
void mcp_handler_process_message(mcp_handler_t *mcp, const char *json, size_t len) {
    ...
    if (strcmp(type_str, "mcp") == 0) {
        ...
        if (strcmp(method, "tools/call") == 0) {
            ...
            while (pp < p + payload_len && (*pp == ' ' || *pp == ':' || *pp == '\t')) pp++;
            if (pp >= p + payload_len || *pp != '{') return;
        ...
        while (c < json + len && (*c == ' ' || *c == ':' || *c == '\t')) c++;
        if (c >= json + len || *c != '[') return;
        ...
        while (item < arr_end - 1) {
            ...
            if (item >= arr_end - 1 || *item != '{') break;

            /* 通过括号深度匹配找到命令对象的结束位置 */
            const char *item_end = item;
            int item_depth = 1;
            while (item_end < arr_end - 1 && item_depth > 0) {
                if (*item_end == '{') item_depth++;
                else if (*item_end == '}') item_depth--;
                item_end++;
            }
